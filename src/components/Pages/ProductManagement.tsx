...
export default function ProductManagement() {
  ...
  const openEdit = (p: Product): void => {
    ...
    setForm({
      ...
      sizes: p.sizes?.join(", ") || "",
      colors: p.colors?.join(", ") || "",
      season: p.season || "", // Added season field
    });
    setSelectedFiles([]);
    setImagePreviews(p.images || []);
    setShowModal(true);
  ...
  const handleSave = async (): Promise<void> => {
    ...
    if (!form.season) {
      alert("Please select a season collection");
      return;
    }
    ...
    try {
      ...
      const productData = {
        ...
        sizes: form.sizes.split(",").map((s) => s.trim()).filter(Boolean),
        colors: form.colors.split(",").map((c) => c.trim()).filter(Boolean),
        season: form.season, // Added season field
        images: imageUrls,
      };
  ...
  // Define form fields including season as select dropdown
  const formFields: {
    label: string;
    key: keyof ProductForm;
    placeholder: string;
    ...
    { 
      ...
      required: true,
      options: [
        { value: "summer", label: "☀️ Summer Collection" },
        { value: "winter", label: "❄️ Winter Collection" },
        { value: "rainy", label: "🌧️ Rainy Season" },
        { value: "all", label: "🔄 All Seasons (Year-round)" }
      ]
    },
  ];

  // Helper function to get season badge styling
  const getSeasonBadge = (season?: string) => {
    if (!season) return { emoji: "📅", label: "Not Set", className: "bg-gray-50 text-gray-700" };
    
    switch(season) {
      case 'summer':
        return { emoji: "☀️", label: "Summer", className: "bg-yellow-50 text-yellow-700 border border-yellow-200" };
      case 'winter':
        return { emoji: "❄️", label: "Winter", className: "bg-blue-50 text-blue-700 border border-blue-200" };
      case 'rainy':
        return { emoji: "🌧️", label: "Rainy", className: "bg-green-50 text-green-700 border border-green-200" };
      case 'all':
        return { emoji: "🔄", label: "All Seasons", className: "bg-purple-50 text-purple-700 border border-purple-200" };
      default:
        return { emoji: "📅", label: season, className: "bg-gray-50 text-gray-700" };
    }
  };
          ...
          <div className="bg-white rounded-2xl border border-gray-100 shadow-sm overflow-hidden">
            ...
            <div className="hidden sm:block overflow-x-auto">
              <table className="w-full">
                <thead>
                  <tr className="border-b border-gray-100">
                    {["Product", "Description", "Category", "Season", "Price", "Stock", "Status", "Actions"].map((h) => (
                      <th
                        key={h}
                        className={`text-left text-xs font-semibold text-gray-400 uppercase tracking-wider py-4 ${
                          h === "Product" ? "px-6" : h === "Actions" ? "px-6 text-right" : "px-4"
                  ...
                  {paginated.map((product) => {
                    ...
                    return (
                      <tr key={product._id} className="hover:bg-gray-50/60 transition-colors group">
                        ...
                        <td className="px-4 py-4 text-sm text-gray-500 capitalize">{product.category}</td>
                        <td className="px-4 py-4">
                          <span className={`inline-flex items-center gap-1 px-2 py-1 rounded-full text-xs font-semibold ${seasonBadge.className}`}>
                            <span>{seasonBadge.emoji}</span>
                    ...
                    <div className="flex gap-3">
                      ...
                      <div className="flex-1 min-w-0">
                        <div className="flex items-start justify-between gap-2">
                          ...
                          <span className={`inline-flex items-center gap-1 px-2 py-0.5 rounded-full text-xs font-semibold ${seasonBadge.className}`}>
                            <span>{seasonBadge.emoji}</span>
                            <span>{seasonBadge.label}</span>
                          </span>
                        </div>
                        ...
                        <div className="flex items-center gap-2 mt-2 text-xs">
                          ...
                          <span className="text-gray-500">{product.stock} units</span>
                          <span>•</span>
                          <span className={`inline-flex items-center px-1.5 py-0.5 rounded-full text-xs font-semibold ${STATUS_STYLES[getStatus(product.stock)]}`}>
                            {getStatus(product.stock)}
                          </span>
                        </div>
                        <div className="flex gap-2 mt-3">
      ...
      {showModal && (
        <div className="fixed inset-0 bg-black/50 z-50 flex items-center justify-center p-4 overflow-y-auto">
          <div className="bg-white rounded-2xl shadow-2xl w-full max-w-2xl my-8">
            ...
            <div className="p-6 space-y-4 max-h-[70vh] overflow-y-auto">
              {formFields.map(({ label, key, placeholder, type = "text", required, options }) => (
                <div key={key}>
                  <label className="block text-xs font-semibold text-gray-500 mb-1.5">
                    {label} {required && <span className="text-red-500">*</span>}
                  </label>
                  ...
                  {key === "season" && (
                    <p className="text-xs text-gray-400 mt-1">
                      💡 Select which season this product is best suited for
                    </p>
                  )}
                </div>
              ))}
